...
import { MapPin, Star, Building2, Plus, Trash2 } from 'lucide-react';
...
const VendorDirectory = () => {
  ...
  const [newVendor, setNewVendor] = useState({ name: '', location: '', rating: '' });
  ...
  const handleAddVendor = async () => {
    ...
    try {
      ...
      setNewVendor({ name: '', location: '', rating: '' });
      fetchVendors();
    } catch (error) {
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2"><Label>Initial Rating (1-5)</Label><Input type="number" step="0.1" value={newVendor.rating} onChange={(e) => setNewVendor({...newVendor, rating: e.target.value})} /></div>
            ...
            <motion.div key={vendor.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.1 }}>
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-sm font-semibold"><Star className="h-4 w-4 text-yellow-500 fill-yellow-500" /> {vendor.rating || 'N/A'}</div>
                </CardContent>
              </Card>
            </motion.div>
          ))}
